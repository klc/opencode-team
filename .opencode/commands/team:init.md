...
## Phase 5c — Graphify Setup

Check if graphify is already installed:

```bash
graphify --help > /dev/null 2>&1 && echo "installed" || echo "missing"
```

**If missing**, inform the user:

```
⚠️  graphify is not installed.
Install it with:

  uv tool install graphifyy && graphify opencode install
  # or: pipx install graphifyy && graphify opencode install

Then re-run /team:init to complete setup.
```

**If installed**, run the OpenCode integration:

```bash
graphify opencode install
```

This writes the graphify section to `AGENTS.md` and registers the `tool.execute.before` plugin.

Then verify `.graphifyignore` exists:

```bash
test -f .graphifyignore && echo "exists" || echo "missing"
```

**If missing**, create `.graphifyignore`:
...
# Graphify own outputs (prevents infinite loop)
graphify-out/

# Team memory & task tracking (not code — graph not needed)
.kanban/
.memory/
...
Run /graphify in this chat to build the initial graph (~2-3 min, one-time LLM cost).
After that, run `graphify update .` after code changes — instant, no API cost.
```
...
Graphify:
  ...
  Graph:    run /graphify to build (one-time)
...
Next:
  ...
  2. Open AGENTS.md and fill in your project rules
