...
description: Load when you receive code review feedback that requires fixes. Enforces: verify before implementing, ask before assuming, one item at a time, technical pushback is allowed.
---
...
- Check the project-stack skill for constraints that might affect the suggestion
...
2. Load `verification-before-completion` skill — verify each fix actually addresses the review item
