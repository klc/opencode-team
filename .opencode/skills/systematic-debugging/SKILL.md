...
description: Load when investigating any bug, test failure, unexpected behavior, or production incident. Mandates root cause investigation before any fix attempt. If you haven't completed Phase 1, you cannot propose fixes.
---
...
1. **Create a failing test case first** (use `test-driven-development` skill)
...
| "One more fix" (after 2+ failures) | 3+ failures = architectural problem. Stop fixing, question the design. |
...
## For the Debugger Agent

When you receive a bug report:

1. Do NOT propose fixes in your initial response
2. Complete Phase 1 and 2 first
3. State your confirmed root cause with evidence
4. Only then propose a fix
5. Your fix recommendation should reference the specific file and line
