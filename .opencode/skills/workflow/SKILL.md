...
lead → @tester (one per independent scope, in parallel)
    ↓
If FAIL → lead reassigns fix to developer → re-runs review → re-runs tester for that scope
    ↓
...
Before starting any implementation or investigation task, load codebase context in this order — cheapest first:

| Step | Action | Token Cost |
|---|---|---|
| 1 | `cat graphify-out/GRAPH_REPORT.md` (if exists) | ~150 — replaces reading dozens of files |
| 2 | `graphify query "<your question>"` | ~100 per query |
| 3 | `memory_search({ query: "..." })` | ~200 |
| 4 | Read only the **specific file(s)** graphify pointed to | targeted |

**Rule:** Never read a full directory of files upfront. Use graphify to find which files matter, then read only those.

**When graphify-out/ does not exist yet:** Type `/graphify` in chat to trigger the first build (one-time, ~2-3 min). After that, `graphify update .` keeps it current after any code changes.

**After modifying code:** Run `graphify update .` before reporting completion to the lead. This is AST-only — no API cost.
