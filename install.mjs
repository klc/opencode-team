...
const MODEL_TIER_BY_AGENT = {
  ...
  'performance-analyst': 'strong',

  'project-manager': 'medium',
  'frontend-lead': 'medium',
  'senior-backend': 'medium',
  'senior-frontend': 'medium',
  'tester': 'medium',
  'code-reviewer': 'medium',

  'junior-backend': 'fast',
  'junior-frontend': 'fast',
  'researcher': 'fast',
  'librarian': 'fast',
}

const DEFAULT_TIER_MODELS = {
  strongModel: 'anthropic/claude-opus-4-5',
  mediumModel: 'anthropic/claude-sonnet-4-5',
  fastModel: 'anthropic/claude-haiku-4-5',
}

function buildAgentModelsByTier({ strongModel, mediumModel, fastModel }) {
  const tierModels = { strong: strongModel, medium: mediumModel, fast: fastModel }
  return Object.fromEntries(AGENT_ORDER.map(agent => [agent, tierModels[MODEL_TIER_BY_AGENT[agent]]]))
}
...
function writeOpencodeJson(destPath, agentBlock, isProject) {
  ...
  config.plugin = [...new Set([...existingPlugins, ...requiredPlugins])]
...
async function main() {
  ...
  console.log('')
  console.log(`  ${bold('1) Quick config')}    — choose three models: strong, medium, and fast.`)
  console.log(`     ${dim('The installer assigns them to the team by role risk and expected request volume.')}`)
  console.log('')
  console.log(`  ${bold('2) Detailed config')} — choose a model for every agent one by one.`)
  console.log(`     ${dim('Use this when you want exact per-agent control over cost and capability.')}`)
  console.log('')

  const configChoice = await ask('Choice [1/2]', '1')
  const useDetailedConfig = configChoice === '2'

  let agentModels
  if (useDetailedConfig) {
    step('Detailed model assignments')
    console.log('')
    console.log(`  ${dim('Choose a model for every agent. Each prompt starts with the recommended tier default for that role.')}`)
    agentModels = buildAgentModelsByTier(DEFAULT_TIER_MODELS)
    ...
    const strongModel = await selectModel('Strong model', DEFAULT_TIER_MODELS.strongModel, models)
    console.log('')
    const mediumModel = await selectModel('Medium model', DEFAULT_TIER_MODELS.mediumModel, models)
    console.log('')
    const fastModel = await selectModel('Fast model', DEFAULT_TIER_MODELS.fastModel, models)

    agentModels = buildAgentModelsByTier({ strongModel, mediumModel, fastModel })
  }
